...
**On fine-tuning (PEFT/LoRA/QLoRA):** every model improvement in this system to date — including the Intent Router's v1→v2 fix (decision D035) — has been prompt engineering (editing the text sent to the model, versioned as a file), not fine-tuning (updating the model's actual weights). That's not an oversight; it's the only option available today. GPT-4o-mini/GPT-4o are closed-weight models reached only through OpenAI's API — there are no weight tensors to attach a LoRA adapter to, and OpenAI's own hosted fine-tuning endpoint isn't PEFT/LoRA in the technical sense (it's an opaque managed service). Genuinely applying PEFT (freezing the base model, training only a small injected set of parameters), LoRA (low-rank adapter matrices approximating a full weight update at a fraction of the parameter count), or QLoRA (LoRA on top of a 4-bit-quantized frozen base, enabling fine-tuning of large models on a single consumer GPU) would require the provider-agnostic step above to reach an open-weight model (Llama, Mistral, Qwen) run on infrastructure this project controls, plus a library like Hugging Face's `peft`. Worth pursuing only if a real capability gap shows up that prompt engineering can't close — a specific, high-volume output format, or domain adaptation beyond what in-context examples can teach — not adopted for its own sake.
...
**Next step:** Layered feedback beyond a single binary signal — e.g. per-recommendation feedback (which of the 3 cards actually landed), a free-text comment box wired to the already-existing `feedback_text` column, or a counselor/parent review queue for flagged (high-risk guardrail) responses. Each of these extends a mechanism that already exists end-to-end in the database; none require new schema design from scratch. The counselor/parent review queue specifically depends on Real Authentication below — there's no way to scope "this parent's queue" to the right student without a verified identity behind it.

## Real Authentication (OAuth/SSO)

**Today:** A student is "recognized" by whatever name string they type into a text box — `StudentRepository.create_or_get_student()` does a case-insensitive lookup-or-create by name, with no password, no verification, and nothing stopping one person from typing another student's name and reading their profile and history. Decision D036 made the failure mode of *not* entering a name explicit (chat now refuses to proceed rather than silently sharing a "Student" identity across every anonymous visitor), but that fix narrows the blast radius of no-name usage — it doesn't add real identity verification for named usage at all.

**Next step:** OAuth/SSO through the identity provider a school already issues accounts with (Google Workspace for Education or Microsoft 365 Education), rather than a self-managed username/password system — students wouldn't create a new credential, and the school's own account security (password policy, MFA, deprovisioning when a student leaves) becomes PathFinder's security too. Streamlit's native `st.login()` (1.42+) handles the OIDC redirect/callback flow directly, which is the natural fit here rather than reaching for a third-party auth library. The architecturally significant part isn't the login screen — it's that `student_id` currently *is* the name match; real auth requires re-keying identity off the provider's stable, unique `sub` claim instead, so `name` becomes a changeable display field rather than the actual database key. That touches `StudentRepository`, `MemoryAgent`, and every `student_name` parameter threaded through `orchestrator.run_turn()`. Role fields (student / parent / counselor) and the access boundaries they imply (a parent scoped to their own child, not a global name lookup) are a natural extension once real identity exists, and are what actually makes the HITL review queue above meaningful rather than an unenforced convention. Real accounts holding verified student identities also start to raise FERPA/COPPA-adjacent data-handling questions (retention policy, parental consent for minors) worth scoping explicitly rather than carrying over silently from the current no-auth prototype.
