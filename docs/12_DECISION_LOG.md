...
Live usage exposed two failures: asking for a roadmap of an already-offered career triggered an unwanted fresh recommendation set, and an implicit reference ("colleges for same") resolved to the wrong career. Root cause: `_match_previous_choice()` only recognized exact literal title mentions, and conversation history was never actually wired into the recommendation/discovery prompts despite the plumbing existing for it. Fixed with a new 11th agent, `IntentRouterAgent` (`src/agents/intent_router_agent.py`, prompt v1), which classifies each turn into `explore` (new recommendations, full pipeline), `roadmap` (reuses last turn's items verbatim), `related_topic` (grounds retrieval in a resolved anchor), or `general_chat` (direct answer, no recommendation structure); an unresolvable classification falls back to `explore`. `career`/`college` were deliberately kept as one unified `related_topic` intent rather than split, and GPA-based reach/target/likely banding was declined as out of scope. Verified live end-to-end by replaying the exact reported conversation - all three problem cases now resolve correctly; full acceptance suite passed (7/7).
...
Added `src/scripts/test_intent_router.py`, a 20-case golden-dataset script with a real confusion matrix and per-class precision/recall/F1 - the only metric-based test in the project, since intent classification is the one stage with genuine checkable ground truth. It immediately surfaced a bug the acceptance suite couldn't: the model correctly resolved implicit references ("that one", "same") but echoed `anchor_title` with a "(type)" suffix, which strict exact-match validation then rejected, silently misrouting the turn to `explore`. Fixed two ways: prompt v2 added a worked example of correct vs. incorrect output, and `_validate()` gained a `_strip_type_suffix()` normalization backstop. Also fixed a separate gap where `general_chat` turns weren't traced at all. Verified: 100% (20/20) across 3 consecutive post-fix runs; added explicit pass/fail thresholds (90% overall accuracy, 90% anchor accuracy, 80% per-class recall floor) with a printed `VERDICT: PASS`/`FAIL` line.

---

## D036: Chat is blocked entirely until a name is entered - anonymous visitors were silently sharing one identity

Investigating a live report of a truncated response led to a real, separate bug in `chat.py`: any visitor who submitted a message without entering a name was silently assigned the literal name `"Student"`, and since student lookup is case-insensitive by name, every anonymous visitor collided onto the same `student_id` - inheriting a stranger's profile, history, and recommendations. (The truncated-response report itself was first misattributed to a stale process; that diagnosis was wrong and is corrected in D037.) Fixed by blocking the turn entirely rather than picking a better placeholder: `chat.py` now shows a warning and refuses to call `run_turn()` until a name is entered - no message saved, no student row touched. A per-session random ID was considered and rejected, since it wouldn't persist across visits and would just trade one identity bug for another. Verified live: no name blocks the turn with a warning; entering one proceeds normally through the full pipeline.

## D037: A fifth intent ("suggest"), a chat-rendering bug that fully explains D036's original report, and clickable next-step chips

Added a fifth intent, `suggest` (prompt v3), for pure interest-sharing ("I like gaming") with no explicit ask - a short 2-4 direction reply with no colleges, no `RecommendationAgent` call, no path plan - while an explicit ask or uncertainty language ("I don't know what career I want") still escalates to `explore`. Two regressions surfaced during real testing: the suggest/explore boundary was initially too broad, misclassifying uncertainty-seeking language as `suggest` (fixed with prompt v4, worked examples added); and the capstone's own scripted demo line - genuine interest-sharing with no uncertainty phrase - broke because it now read as a textbook `suggest` case, resolved by a deliberate rule that `suggest` never fires on a student's first message (enforced in code, not just the prompt). A third, more serious bug turned up while investigating the demo regression: `chat.py`'s renderer built its displayed text from a template that only works when there are recommendation cards to introduce, so `general_chat`, `suggest`, and `roadmap` turns - which intentionally have none - had been silently showing just "Thanks, [name]!" with the real answer dropped, since D034 shipped. This fully explains D036's original report, which had been misdiagnosed as a stale process. Fixed by falling back to the full response text whenever there are no cards. Clickable next-step chips (e.g., "Show me college options for `<X>`") were added on top of the now-working foundation, reusing the same message-handling path as typed input. Verified end-to-end live in the browser after all fixes; re-ran the full acceptance suite (7/7) and the intent router golden dataset (extended to 27 cases, 92.6% accuracy, passing the D035 thresholds).

---

## D038: Retrieval and recommendation now respect an explicit "colleges only" or "careers only" ask, instead of always returning a fixed blend

Live usage surfaced a real gap: asking "recommend some colleges" returned no college cards at all, just prose mentioning college names. Root cause was two-fold. First, a stale server process was still running the pre-D037 `intent_router` prompt, misrouting the message to `general_chat` (no anchor could be confidently resolved against prior recommendations under the older prompt), which never produces structured cards - fixed by restarting. Second, and the real underlying gap: even under `explore`/`related_topic`, `RetrievalAgent` always split retrieval into a fixed 2-of-5 college slots regardless of what was asked, and `RecommendationAgent`'s prompt had no signal for what type the student wanted, so the model had no reason to prefer colleges even when asked directly.

Fixed by extending `IntentRouterAgent`'s existing classification call (no added API call) with a `requested_type` field - `"college"`, `"career_or_major"`, or `"any"` - added in prompt v4 -> v5. `RetrievalAgent.retrieve_relevant_context()` now allocates all slots to colleges (or none) when `requested_type` says so, instead of always blending; `RecommendationAgent.generate_recommendations()` now passes an explicit instruction ("every recommendation must be type X") into the prompt rather than leaving the model to guess from a blended context. Verified live: "recommend some colleges" now correctly classifies `requested_type: "college"` and returns 3/3 `college_pathway` recommendations with real named schools; "give me career options" in a fresh conversation returns 3/3 career/major recommendations, no colleges. Re-ran the intent router golden dataset (96.3%, passing D035's floors) and the full acceptance suite (7/7) - no regression.
